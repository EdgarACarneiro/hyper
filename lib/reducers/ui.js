...
const reducer = (state = initial, action) => {
  ...
  switch (action.type) {
      ...
      state_ = state
        ...
        .merge(
          (() => {
            ...
            /*if (config.globalZoom) {
              ret.globalZoom = config.globalZoom;
            }*/
            //TODO THISSSSS
            ret.globalZoom = false;
    ...
    case SESSION_ADD:
    console.log("HERE "); console.log(state);
      state_ = state
        .merge(
          {
        ...
        //For assiociationg a pane to its fontsize
        .setIn(['panesFontSize', action.uid], PaneFontSize({paneFontSize: state.fontSize}));
      break;
    ...
    case UI_FONT_SIZE_SET:
      if (state.globalZoom) state_ = state.set('fontSizeOverride', action.value);
      else {
        let updatedPane = state.panesFontSize[state.activeUid].set('paneFontSize', action.value);
        let updatedFontSizes = state.panesFontSize.set(state.activeUid, updatedPane);
        state_ = state.set('panesFontSize', updatedFontSizes);
      }
      break;

    case UI_FONT_SIZE_RESET:
      if (state.globalZoom) state_ = state.set('fontSizeOverride', null);
      else {
        let updatedPane = state.panesFontSize[state.activeUid].set('paneFontSize', state.fontSize);
        let updatedFontSizes = state.panesFontSize.set(state.activeUid, updatedPane);
        state_ = state.set('panesFontSize', updatedFontSizes);
      }
      break;
