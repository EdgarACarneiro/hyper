...
import {
  ...
  UI_FONT_SIZE_INCR,
  UI_FONT_SIZE_DECR,
  //UI_PANE_FONT_SIZE_SET,
  UI_PANE_FONT_SIZE_INCR,
  UI_PANE_FONT_SIZE_DECR,
  UI_FONT_SIZE_RESET,
...
export function increasePaneFontSize() {
  return (dispatch, getState) => {
    dispatch({
      type: UI_PANE_FONT_SIZE_INCR,
      effect() {
        const state = getState();
        const old = state.ui.fontSizeOverride || state.ui.fontSize;
        const value = old + 1;
...
//TODO: falta fazer a UI_PANE_FONT_SIZE_SET, que recebe um id/ cena correspondente a um pane e so faz zoom ai

export function decreasePaneFontSize() {
  return (dispatch, getState) => {
    dispatch({
      type: UI_PANE_FONT_SIZE_DECR,
      effect() {
        const state = getState();
        const old = state.ui.fontSizeOverride || state.ui.fontSize;
        const value = old - 1;
