  ...
  return (dispatch, getState) => {
    dispatch({
      ...
      effect() {
        const state = getState();
        let old;
        if (state.ui.globalZoom) old = state.ui.fontSizeOverride || state.ui.fontSize;
        else {
          const currentPane = state.sessions.activeUid;
          old = state.ui.panesFontSize[currentPane].paneFontSize;
        }
        const value = old + 1;
        dispatch({
