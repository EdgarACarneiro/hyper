...
export default class Term extends PureComponent {
  ...
  template(css) {
    let currFS;
    if (this.props.globalZoom) currFS = this.props.fontSize;
    else currFS = this.props.panesFontSize[this.props.uid].paneFontSize;

    return (
      <div
        className={css('fit', this.props.isTermActive && 'active')}
        style={{padding: this.props.padding, fontSize: currFS}}
