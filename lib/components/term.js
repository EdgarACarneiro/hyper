...
export default class Term extends PureComponent {
  constructor(props) {
    ...
    //Local Variables for Font Size Handling
    this.localFontOverride = 0; //Alterar estes valor e usar no render conforme o keymap e detectado
    //throw (null);
  }
  ...
  keyboardHandler(e) {
    ...
    //Pane Zoom In Keymap - hardcoded for now
    if (e.ctrlKey && e.shiftKey && (e.key == '*')) {
      console.log("Zooming in pane");
      this.localFontOverride++;
    }
    else
    //Pane Zoom Out Keymap - hardcoded for now
    if (e.ctrlKey && e.shiftKey && (e.key == '_')) {
      console.log("Zooming out pane");
      //this.localFontOverride--;
      console.log(this.localFontOverride);
    }
  ...
  template(css) {
    return (
      <div
        ...
        style={{padding: this.props.padding, fontSize: this.props.fontSize + this.localFontOverride}}
